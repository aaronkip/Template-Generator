...
class WidgetCategory {
  ...
  /// platManualModPage in list format.
  ///
  /// Eg: In json file we can specify like below,
  ///
  /// "platManualModPageToHide": ["linux", "android"] => the specific category should not show on the linux and android platManualModPage
  final List<dynamic>? platManualModPageToHide;
}
...
class Control {
  ...
  /// Eg: In json file we can specify like below,
  ///
  /// "platManualModPageToHide": ["linux", "android"] => the current control should not show on the linux and android platManualModPage
  final List<dynamic>? platManualModPageToHide;
}
...
class SubItem {
  ...
  /// Eg: In json file we can specify like below,
  ///
  /// "platManualModPageToHide": ["linux", "android"] => the specific sample should not show on the linux and android platManualModPage
  final List<dynamic>? platManualModPageToHide;
}
...
Future<void> updateControlItems() async {
  ...
  for (int index = 0; index < categoryList.length; index++) {
    ...
    if (SampleModel._categoryList[index].platManualModPageToHide == null ||
        _needToShow(SampleModel._categoryList[index].platManualModPageToHide)) {
      for (int i = 0;
          i < SampleModel._categoryList[index].controlList!.length;
          i++) {
        controlList.add(
            Control.fromJson(SampleModel._categoryList[index].controlList![i]));
        if (controlList[i].platManualModPageToHide == null ||
            _needToShow(controlList[i].platManualModPageToHide)) {
          for (int j = 0; j < controlList[i].subItems!.length; j++) {
            ...
            if (_firstLevelSubItems[j].type == 'parent') {
              ...
              if (_firstLevelSubItems[j].platManualModPageToHide == null ||
                  _needToShow(_firstLevelSubItems[j].platManualModPageToHide)) {
                _isChild = true;
                for (int k = 0;
                    k < _firstLevelSubItems[j].subItems!.length;
                    k++) {
...
bool _needToShow(List<dynamic>? platManualModPage) {
  return !((platManualModPage!.contains('web') && kIsWeb) ||
      (!kIsWeb &&
          ((platManualModPage.contains('linux') && Platform.isLinux) ||
              (platManualModPage.contains('android') && Platform.isAndroid) ||
              (platManualModPage.contains('iOS') && Platform.isIOS) ||
              (platManualModPage.contains('windows') && Platform.isWindows) ||
              (platManualModPage.contains('macOS') && Platform.isMacOS))));
}
